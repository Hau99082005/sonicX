...
export const updatedFollower: RequestHandler = async (req, res) => {
    ...
    if (alreadyFollower) {
        ...
        await User.findByIdAndUpdate(req.user.id, { $pull: { followings: profileId } });
        status = "removed";
    } else {
        await User.findByIdAndUpdate(profileId, { $addToSet: { followers: req.user.id } });
        await User.findByIdAndUpdate(req.user.id, { $addToSet: { followings: profileId } });
        status = "added";
    }
...
export const getFllowersProfile: RequestHandler = async (req, res) => {
    const { limit = "80", pageNo = "0" } = req.query as paginationQuery;
